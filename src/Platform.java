...
public class Platform extends DataConstants {
	...
	/**
	 * Mainstage uses three role based UI interactions. Each feature calls another
	 * method which also uses role based calls. Guest: When a user is not logged in.
	 * They can Login, Register, View Listings, and Search Listings Leasee: When a
	 * user has the type "Leasee" They can Search listings, View leases, View
	 * favorite listings, Review listings, Manage account, and logout. Landlord:
	 * When a user has the type "Landlord" They can view listings, view leases,
	...
	private static void mainStage() {
		...
		for (int i = 0; i < userList.size(); i++) {
			if (userList.get(i).getType().equalsIgnoreCase("Leasee")) {
				leasee = new Leasee(userList.get(i).getId(), userList.get(i).getFirstName(),
						userList.get(i).getLastName(), userList.get(i).getAddress(), userList.get(i).getEmail(),
						userList.get(i).getPassword(), userList.get(i).getType(), null);
			} else {
				landlord = new Landlord(userList.get(i).getId(), userList.get(i).getFirstName(),
						userList.get(i).getLastName(), userList.get(i).getAddress(), userList.get(i).getEmail(),
						userList.get(i).getPassword(), userList.get(i).getType());
			}
		}
		while (guest) {
	...
	/**
	 * Login method: Checks if the user is a guest (avoid repeat logins) Takes
	 * username and password and checks the user list, determines their type ands
	 * then creates the respective object (leasee/landlord)
	 */
			...
			for (int i = 0; i < userList.size(); i++) {
				if ((userList.get(i).getId().equals(username)) && (userList.get(i).getPassword().equals(password))) {
					...
					if (user.getType().equalsIgnoreCase("Leasee"))
						leasee = new Leasee(user.getId(), user.getFirstName(), user.getLastName(), user.getAddress(),
								user.getEmail(), user.getPassword(), user.getType(), null);
					else if (user.getType().equalsIgnoreCase("Landlord"))
						landlord = new Landlord(user.getId(), user.getFirstName(), user.getLastName(),
								user.getAddress(), user.getEmail(), user.getPassword(), user.getType());
				}
				guest = false;
			}
			if (guest) {
	...
	/**
	 * Search listings has searching by address, ammentities, price, distance from
	 * russel house, and description search
	 */
		...
		for (Listing listing : listings.getListings()) {
			if (listing.getListingId() == listingId) {
				...
				System.out.println("\nWasher: " + (listing.isAmenitiesWasher() ? "Yes" : "No") + "\nAir Conditioning: "
						+ (listing.isAmenitiesAC() ? "Yes" : "No") + "\nFurniture: "
						+ (listing.isAmenitiesFurniture() ? "Yes" : "No") + "\nPatio: "
						+ (listing.isAmenitiesPatio() ? "Yes" : "No") + "\nDishwasher: "
						+ (listing.isAmenitiesDishwasher() ? "Yes" : "No") + "\nFireplace: "
						+ (listing.doesAllowPets() ? "Yes" : "No") + "\nWi-Fi: "
						+ (listing.isAmenitiesWifi() ? "Yes" : "No") + "\nPool: "
						+ (listing.isAmenitiesPool() ? "Yes" : "No"));
	...
	/**
	 * Post a listing using address, description, distance from Russell house,
	 * monthly rent price, availability, and all of the amenities
	 */
	private static void postListing() {
		...
		if (scan.nextLine().equalsIgnoreCase("y")) {
			...
			listingList.add(new Listing((listingList.size() + 1), address, beds, bathrooms, distanceFromRussellHouse,
					price, available, landlord.getId(), amenitiesWasher, amenitiesAC, amenitiesFurniture,
					amenitiesPatio, amenitiesDishwasher, amenitiesFireplace, amenitiesWifi, amenitiesPool));
					...
					if (ID == listingList.get(i).getListingId()) {
						...
						for (int k = 0; k < leaseeList.size(); k++) {
							if (answer.equalsIgnoreCase(leaseeList.get(k).getId())) {
								String leasee2Name = leaseeList.get(k).getFirstName() + " "
										+ leaseeList.get(k).getLastName();
								System.out.println("You are about to sign a lease for "
										+ listingList.get(i).getAddress() + " type y to continue");
								String response = scan.nextLine();
								if (response.equalsIgnoreCase("y")) {
									Lease l1 = new Lease(ID, listingList.get(i).getLandlordId(), user.getId(),
											"10/31/20", listingList.get(i).getAddress());
									String lease = "This lease Agreement is made and entered on " + l1.getDate()
									+ " by and between " + listingList.get(i).getLandlordId() + " and "
									+ leasee.getFirstName() + " " + leasee.getLastName() + " " + LEASEPT1 + " "
									+ listingList.get(i).getBeds() + " bedrooms and"
									+ listingList.get(i).getBeds() + " bathrooms, located at "
									+ listingList.get(i).getAddress() + " 29208.  " + LEASEPT2 + " 11/1/20"
									+ " to " + "11/1/21. \r\n" + "\r\n4.  Rent. The Tenant will pay $"
									+ listingList.get(i).getPrice()
									+ " each month on the first of the month.\r\n \r\n5.  Payment should be sent to: "
									+ landlord.getAddress()
									+ "\r\n \r\n6.  Damages. Charges will be billed to the client for damaged property, up to $5000"
									+ "\r\n \r\n7.  Signatures\n" + LEASESIGN + leasee.getFirstName() + " "
									+ leasee.getLastName() + LEASESIGN + leasee2Name + LEASESIGN
									+ listingList.get(i).getLandlordId();
						...
						if (answer.equalsIgnoreCase("no")) {
							System.out.println("You are about to sign a lease for " + listingList.get(i).getAddress()
									+ " type y to continue");
							scan.nextLine();
							String response = scan.nextLine();
							if (response.equalsIgnoreCase("y")) {
								Lease l1 = new Lease(ID, listingList.get(i).getLandlordId(), user.getId(), "10/31/20",
										listingList.get(i).getAddress());
								String lease = "This lease Agreement is made and entered on " + l1.getDate()
								+ " by and between " + listingList.get(i).getLandlordId() + " and "
								+ leasee.getFirstName() + " " + leasee.getLastName() + " " + LEASEPT1 + " "
								+ listingList.get(i).getBeds() + " bedrooms and" + listingList.get(i).getBeds()
								+ " bathrooms, located at " + listingList.get(i).getAddress() + " 29208.  "
								+ LEASEPT2 + " 11/1/20" + " to " + "11/1/21. \r\n"
								+ "\r\n4.  Rent. The Tenant will pay $" + listingList.get(i).getPrice()
								+ " each month on the first of the month.\r\n \r\n5.  Payment should be sent to: "
								+ landlord.getAddress()
								+ "\r\n \r\n6.  Damages. Charges will be billed to the client for damaged property, up to $5000"
								+ "\r\n \r\n7.  Signatures\n" + LEASESIGN + leasee.getFirstName() + " "
								+ leasee.getLastName() + LEASESIGN + listingList.get(i).getLandlordId();
								DataWriter.createFile(lease);
								System.out.println("Lease file created, Congrats!");
							}
						}
		...
		if (leasee.getType().equalsIgnoreCase("leasee")) {
			...
			if (messageWindow == 0) {
				...
				if (userInput >= 1 && userInput <= 6) {
					...
					System.out.println(
							"Landlord ID: " + leasee.getLeases().get(startingWindow + (userInput - 1)).getLandlordId());
					System.out.println(
			...
			if (messageWindow == 0) {
				...
				if (userInput >= 1 && userInput <= 6) {
					...
					System.out.println(
							"Date Signed: " + landlord.getLeases().get(startingWindow + (userInput - 1)).getDate());
				} else if (userInput == 7) {
					if (startingWindow > 6) {
						startingWindow -= 6;
						fetchLeases(startingWindow, 6);
	...
	/**
	 * Fetches the leases For leasees, prints address and duration For landlords,
	 * prints out leaseID and duration
	 * 
	 * @param windowStart
	 * @param howMany
	 */
