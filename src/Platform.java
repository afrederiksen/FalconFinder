...
public class Platform extends DataConstants {
  ...
  /**
   * Mainstage uses three role based UI interactions. Each feature calls another method which also
   * uses role based calls. Guest: When a user is not logged in. They can Login, Register, View
   * Listings, and Search Listings Leasee: When a user has the type "Leasee" They can Search
   * listings, View leases, View favorite listings, Review listings, Manage account, and logout.
   * Landlord: When a user has the type "Landlord" They can view listings, view leases, manage
   * account, and logout
   */
  ...
  /**
   * Login method: Checks if the user is a guest (avoid repeat logins) Takes username and password
   * and checks the user list, determines their type ands then creates the respective object
   * (leasee/landlord)
   */
      ...
      for (int i = 0; i < userList.size(); i++) {
        if ((userList.get(i).getId().equals(username))
          ...
          if (user.getType().equalsIgnoreCase("Leasee"))
            leasee = new Leasee(user.getId(), user.getFirstName(), user.getLastName(),
                user.getAddress(), user.getEmail(), user.getPassword(), user.getType(), null);
  ...
  /**
   * Search listings has searching by address, ammentities, price, distance from russel house, and
   * description search
   */
  ...
  /**
   * Post a listing using address, description, distance from Russell house, monthly rent price,
   * availability, and all of the amenities
   */
  private static void postListing() {
    ...
    if (scan.nextLine().equalsIgnoreCase("y")) {
      ...
      listingList.add(new Listing((listingList.size() + 1), address, beds, bathrooms,
          distanceFromRussellHouse, price, available, landlord.getId(), amenitiesWasher,
          amenitiesAC, amenitiesFurniture, amenitiesPatio, amenitiesDishwasher, amenitiesFireplace,
          amenitiesWifi, amenitiesPool));
      ...
      if (user.getType().equalsIgnoreCase("leasee")) {
        ...
        for (int i = 0; i < listingList.size(); i++) {
          if (ID == listingList.get(i).getListingId()) {
            System.out
                .println("Are you signing the Lease alone? If not enter the other leasee's ID");
            scan.nextLine();
            String answer = scan.nextLine();
            for (int k = 0; k < leaseeList.size(); k++) {
              if (answer.equalsIgnoreCase(leaseeList.get(k).getId())) {
                String leasee2Name =
                    leaseeList.get(k).getFirstName() + " " + leaseeList.get(k).getLastName();
  ...
  private static void viewLeases() {
    ...
    if (leasee.getType().equalsIgnoreCase("leasee")) {
      ...
      if (messageWindow == 0) {
        ...
        int userInput = scan.nextInt();
        if (userInput >= 1 && userInput <= 6) {
          System.out.println("Leasee ID: "
              + landlord.getLeases().get(startingWindow + (userInput - 1)).getLeaseeId());
          System.out.println("Date Signed: "
              + landlord.getLeases().get(startingWindow + (userInput - 1)).getDate());
        } else if (userInput == 7) {
          if (startingWindow > 6) {
            startingWindow -= 6;
            fetchLeases(startingWindow, 6);
  ...
  /**
   * Fetches the leases For leasees, prints address and duration For landlords, prints out leaseID
   * and duration
   * 
   * @param windowStart
   * @param howMany
  ...
  private static void fetchLeases(int windowStart, int howMany) {
    if (leasee.getType().equalsIgnoreCase("leasee")) {
      for (int i = 0; i < howMany; i++) {
        System.out
            .println((i + 1) + ". " + leasee.getLeases().get(windowStart + i).getAddress() + " | ");
      }
    } else if (landlord.getType().equalsIgnoreCase("landlord")) {
      for (int i = 0; i < howMany; i++) {
        System.out.println(
            (i + 1) + ". " + landlord.getLeases().get(windowStart + i).getLeaseeId() + " | ");
      }
    }
  }
}
